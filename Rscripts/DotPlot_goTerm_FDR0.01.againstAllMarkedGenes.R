...
comparison <- "MGvsSDL_H2AUB_newDataset"

for (comparison in c("MGvsSDL_H2AUB_newDataset")) {
  ...
  # make result dirs
  GOdir <- paste0("../results/goTerms_FDR0.005")
  ...
  for(i in 1:2){
    j = 3 - i # opposite of i
    peakList <- DEseq2_res_annotated %>% filter(FDR < FDR_th) %>% ## FDR changed to 0.1
  ...
  for(i in 1:2){
    j = 3 - i
    id <- paste0("Up_in_", stage[i], ";comparison", stage[i], "vs", stage[j], ";hist", hist, ".againstAllGenesMarked", ".FDR", FDR_th) # font size 10, wrap 25; font size 8, wrap 40
...
d <- ggplot() +
  ...
  scale_color_gradient(high = "red", low = "blue", limits = range(exp(-16), 0.005), trans = "reverse") # high = smaller p value # exp(-16): to avoid having 0 on the color scale; if a p value small than e-16, it would appear grey
